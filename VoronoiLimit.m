...
if try_ver==1
    try % using hardcoded version (faster)
        ...
        version_online=[str2double(onlinedata(1:ixp(1)-1)) str2double(onlinedata(ixp(1)+1:ixp(2)-1))  str2double(onlinedata(ixp(2)+1:ixp(3)-1)) str2double(onlinedata(ixp(3)+1:end))];
        update_available=0;
        if version_online(1)>version(1)
            ...
            if version_online(2)>version(2)
                ...
                if version_online(3)>version(3)
                    update_available=1;
                else
                    if version_online(4)>version(4)
                        update_available=1;
                    end
                end
            end
